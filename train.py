...
from src import datasets, models, priors
from src.data import DataModule
from src.models import output_modules
from src.models.MABNet.utils import act_class_mapping, rbf_class_mapping
from src.module import LNNP
from src.utils import LoadFromCheckpoint, LoadFromFile, number, save_argparse
...
def get_args():
    ...
    parser.add_argument('--model', type=str, default='MABNet', choices=models.__all__, help='Which model to train')
    ...
    parser.add_argument('--use-padding', action=argparse.BooleanOptionalAction, default=True, help='If true, use padding for the input data')
    ...
    parser.add_argument('--cutoff-pruning', type=float, default=1.6, help='Cutoff pruning in model')
    parser.add_argument('--max-num-edges-save', type=int, default=32, help='Maximum number of edges to save in model')
    ...
    parser.add_argument('--many-body', action=argparse.BooleanOptionalAction, default=True, help='If use many-body interaction, default is True. If False, only use two-body interaction (for ablation study)')
...
def main():
    ...
    dir_name = f"output_ngpus_{len(cuda_visible_devices)}_bs_{args.batch_size}_lr_{args.lr}_seed_{args.seed}" + \
               ...
               f"_L{args.num_layers}_D{args.embedding_dimension}_H{args.num_heads}" + \
               f"_cutoff_{args.cutoff}_cutoff_pruning_{args.cutoff_pruning}_max_num_edges_save_{args.max_num_edges_save}_E{args.energy_weight}_F{args.force_weight}_loss_{args.loss_type}"
