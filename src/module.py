...
class LNNP(LightningModule):
    ...
    def step(self, batch, loss_fn, stage):
        ...
        # batch.y的的dtype如果是torch.float64，则转换为torch.float32
        ...
        # pred的dtype是torch.float64，则转换为torch.float32
        if pred.dtype == torch.float64:
            pred = pred.float()
        ...
        if stage == "test":
            ...
            if self.hparams.derivative:
                ...
                # 如果batch没有dy则是neg_dy
                if "neg_dy" in batch:
                    self.inference_results['dy_true'].append(batch.neg_dy.squeeze(-1).detach().cpu())
                else:
        ...
        if self.hparams.derivative:
            ...
            # 如果batch没有dy则是neg_dy
            if "neg_dy" in batch:
                loss_dy = loss_fn(deriv, batch.neg_dy)
            else:
    ...
    def validation_epoch_end(self, validation_step_outputs):
        if not self.trainer.sanity_checking:
            ...
            # if len(self.losses["train_y"]) > 0 and len(self.losses["train_dy"]) > 0 and len(self.losses["train_out_eef"]) > 0:
